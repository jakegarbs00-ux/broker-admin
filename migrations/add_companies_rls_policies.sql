...
DROP POLICY IF EXISTS "Partners can insert companies" ON companies;
DROP POLICY IF EXISTS "Partners can view companies they referred" ON companies;
DROP POLICY IF EXISTS "Partners can update companies they referred" ON companies;
DROP POLICY IF EXISTS "Clients can insert companies" ON companies;
DROP POLICY IF EXISTS "Clients can view their own company" ON companies;
DROP POLICY IF EXISTS "Clients can update their own company" ON companies;
DROP POLICY IF EXISTS "Admins have full access to companies" ON companies;
...
-- Policy 4: Clients can INSERT companies (for direct signup, not via partner)
CREATE POLICY "Clients can insert companies"
ON companies
FOR INSERT
TO authenticated
WITH CHECK (
  ...
  AND
  -- Company must have referred_by = null (direct signup, not via partner)
  referred_by IS NULL
);

-- Policy 5: Clients can SELECT their own company (via profiles.company_id)
...
-- Policy 6: Clients can UPDATE their own company
