...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, partner_company_id } = body as {
      email?: string;
      partner_company_id?: string;
    };

    if (!email || !partner_company_id) {
      return NextResponse.json(
        { error: 'Email and partner_company_id are required' },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      existingProfile.partner_company_id === partner_company_id
    ) {
      return NextResponse.json(
        { error: 'This user is already linked to this partner company' },
        { status: 400 }
      );
    }
    ...
    // Update profile with PARTNER role and partner_company_id
    ...
    console.error('Error in /api/admin/invite-partner-user:', error);
    return NextResponse.json(
